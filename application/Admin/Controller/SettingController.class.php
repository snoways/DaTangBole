...
class SettingController extends AdminbaseController{
    ...
    public function config()
    {
        ...
        $config = $db->find();
        if (IS_POST){
            $old = $config['vip_radio'];
            $_POST['vip_radio'] = $_POST['vip_radio']/100;
            $db->create();
            $db->where(['id'=>1])->save();

            if ($_POST['vip_radio'] != $old){
                //如果比例变动，需要修改线上课堂的会员价格
                $list = M('OnlineClass')->select();
                foreach ($list as $item){
                    $price = $item['vip_price']*$_POST['vip_radio'];
                    M('OnlineClass')->where(['id'=>$item['id']])->save(['vip_price'=>$price]);
                }
            }
            $this->success('保存成功！');
        }
