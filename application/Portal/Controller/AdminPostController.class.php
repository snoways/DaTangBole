...
//		$this->_getTree();
        ...
        $end_time = I("end_time");
        if($keyword){
            $where['n.news_title'] = array('like','%'.$keyword.'%');
            $this->assign('keyword',$keyword);
        }
        if($cate_id){
            $where['n.cate_id'] = $cate_id;
        ...
        if($start_time){
            $where['n.news_time'] = array('egt',$start_time);
            $this->assign('start_time',$start_time);
            if($end_time){
                $where['n.news_time'] = array('between',array($start_time,$end_time));
                $this->assign('start_time',$start_time);
                $this->assign('end_time',$end_time);
            }
        }elseif($end_time){
            $where['n.news_time'] = array('elt',$end_time);
            $this->assign('end_time',$end_time);
        }


        $count = $this->news_model->alias('n')->join('fzm_news_category c on n.cate_id = c.id')->where($where)->count();
        $page = $this->page($count,15);
        $list = $this->news_model->alias('n')->join('fzm_news_category c on n.cate_id = c.id')->where($where)
            ->limit($page->firstRow,$page->listRows)->order('n.news_time desc')->field('n.*,c.cate_name')->select();
...
//		$this->assign("terms",$terms);
        if(IS_POST){
            $cate_id = I("post.category");
            $news_title = I('post.news_title');
            $news_detail = $_POST['editorValue'];
//            dump($_POST['editorValue']);die;
            if($news_detail == ''){
                $this->error('详情不能为空！');
            }
            $news = $this->news_model->where(array('news_title'=>$news_title))->find();
            if($news){
                $this->error('标题重复，请重新添加！');
            }
            ...
            $info = $this->news_model->add($data);
            if($info){
                $this->success('添加成功！',U('AdminPost/index'));
            }else{
                $this->error('网络错误！');
            }
        }else{
...
//		$this->assign("term",$term_relationship);
        ...
        if(IS_POST){
            ...
            $news = $this->news_model->where($where)->find();
            if($news){
                $this->error('标题重复，请重新添加！');
            }
            $data['cate_id'] = $cate_id;
            $data['news_title'] = $news_title;
            $data['news_detail'] = $news_detail;
            $info = $this->news_model->where(array('id'=>$id))->save($data);
            if($info !== false){
                $this->success('保存成功！',U('AdminPost/index'));
            }else{
                $this->error('网络错误！');
            }
        }else{
            $info = $this->news_model->where(array('id'=>$id))->find();
            $category = $this->category->select();
            $this->assign('category',$category);
	...
	public function delete(){
		...
		if(isset($_GET['id'])){
			$id = I("get.id",0,'intval');
			$find = M('Banner')->where(array('type'=>3,'item_id'=>$id))->find();
			if($find){
				$this->error("文章在使用，不能删除！");
			}
